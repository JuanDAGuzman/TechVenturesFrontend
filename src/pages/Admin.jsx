...
export default function AdminPage() {
  ...
  const [shippingLinkDate, setShippingLinkDate] = useState(
    ...
    <div className="container-page">
      ...
      <section className="card border-2 border-slate-200">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-slate-600">Fecha del envío</label>
            <input
              type="date"
              value={shippingLinkDate}
              onChange={(e) => setShippingLinkDate(e.target.value)}
              className="px-3 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400"
            />
          </div>
          <button
            onClick={() => {
              ...
              {!searchResult.found ? (
                ...
                <>
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    {/* Relación con la tienda: tipo de cita + ciudades de envío */}
                    <div>
                      <h4 className="font-semibold text-sm text-slate-600 mb-3">
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-3">
                        <div className="bg-indigo-50 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-indigo-700">
                            {searchResult.customer.tryout || 0}
                          </div>
                          <div className="text-xs text-indigo-500 mt-1">Pruebas</div>
                        </div>
                        <div className="bg-violet-50 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-violet-700">
                            {searchResult.customer.pickup || 0}
                          </div>
                          <div className="text-xs text-violet-500 mt-1">Recogidas</div>
                        </div>
                        <div className="bg-cyan-50 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-cyan-700">
                            {searchResult.customer.shipping || 0}
                          </div>
                          <div className="text-xs text-cyan-500 mt-1">Envíos</div>
                        </div>
                      </div>
                      {searchResult.customer.cities?.length > 0 && (
                        <div className="flex items-center gap-2 flex-wrap text-sm">
                          <span className="font-medium text-slate-700">Ciudades de envío:</span>
                          {searchResult.customer.cities.map((c) => (
                            <span key={c} className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-medium">
                              {c}
                            </span>
                          ))}
                        </div>
                    ...
                    {searchResult.customer.history?.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-sm text-slate-600 mb-3">
                          Historial Reciente {searchResult.customer.total_appointments > searchResult.customer.history.length
                            ? `(últimas ${searchResult.customer.history.length} de ${searchResult.customer.total_appointments})`
                            : ""}
                        </h4>
                        <div className="border border-slate-200 rounded-xl overflow-hidden">
                          <div className="max-h-64 overflow-y-auto divide-y divide-slate-100">
                            {searchResult.customer.history.map((h, i) => (
                              <div key={i} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
                                <div className="min-w-0">
                                  <p className="font-medium text-slate-700 truncate">{h.product || "Sin producto"}</p>
                                  <p className="text-xs text-slate-400">
                                    {dayjs(h.date).format("DD/MM/YYYY")} · {h.type_code === "TRYOUT" ? "Ensayar" : h.type_code === "PICKUP" ? "Prueba remota" : "Envío"}
                                    {h.shipping_city ? ` · ${h.shipping_city}` : ""}
                                  </p>
                                </div>
                                <span className={`shrink-0 text-[10px] font-bold uppercase px-2 py-1 rounded-full ${
                                  h.status === "CANCELLED" ? "bg-red-100 text-red-700" :
                                  h.status === "NO_SHOW" ? "bg-slate-200 text-slate-600" :
                                  h.status === "DONE" || h.status === "SHIPPED" ? "bg-emerald-100 text-emerald-700" :
                                  "bg-amber-100 text-amber-700"
                                }`}>
                                  {h.status}
                                </span>
